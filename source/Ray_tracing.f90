program Ray_traicing 
  ...
  integer, parameter :: outmin = 0
  integer, parameter :: outmax = 1
  ...
  character(*), parameter :: datadir   = "/storage3/jsmendezh/raytracing/data"
  ! Datafile template
...
subroutine trace()
  ...
  integer :: i, j, k, xi, yi, zi, unitin, i_off, j_off, k_off, ix 
  real :: mintemp, maxtemp, temp, T1, T2, C1, C2, CX
  real :: x, y, z, at, N_sum, ds, s
  real :: r(3), ro(3)
      ...
      do i = 1, ncells_x
        do j = 1, ncells_y
          do k = 1, ncells_z
            ...
            x = ((float(i)-0.5)/p_maxcells_x) * xphystot
            y = ((float(j)-0.5)/p_maxcells_y) * yphystot
            z = ((float(k)-0.5)/p_maxcells_z) * zphystot
            ...
            ro(2) = yo + y
            ro(3) = zo + z
            ! write(*, *) "Posicion de salida del rayo", ro/AU
            ! get the xray emission from physical variables
            !  use subroutine to calculate CX
            ! xray = (CX*(pvars(1)/mu0/AMU)**2)
            ! test
            N_sum = 0
            s = 0
            r = ro
            ! Computational box containment conditions
            ! write(*,*)  "tracing from ro = ", ro(1)/AU, ro(2)/AU, ro(3)/AU 
            do while( r(1) >= 0 .and. r(1) <= xphystot .and. r(2) >= 0 .and. r(2) <= yphystot .and. r(3) >= 0 .and. r(3) <= zphystot )
              ! Run over the ray
              r = ro + s*los
              ! write(*,*) "s = ", s/AU
              ! write(*,*) "r = ", r(1)/AU, r(2)/AU, r(3)/AU 
          
              ! take the density value from max resolution grid 
              xi = int(r(1)/xphystot * p_maxcells_x + 0.5)
              yi = int(r(2)/yphystot * p_maxcells_y + 0.5)
              zi = int(r(3)/zphystot * p_maxcells_z + 0.5)

              ! Get the density value in the mapo at max resolution
              if (zi <= 0) then 
                write(*,*) xi, yi, zi, r(3)/AU, r(3)/zphystot, r(3)/zphystot * p_maxcells_z
                stop
              end if 

              N_sum = N_sum + ds*aux_density(xi, yi, zi)
              ! write(*,*) aux_density(xi, yi, zi)
              ! Increment the step length over the ray 
              s = s + ds
            ...
            ! write(*,*) "projecting ..."
            ! Project cells into 2D output map
            ! given a block and i,j,k tell me global coordinates 

            call absCoords(bID, i, j, k, mesh, i1, j1, k1)
            
            ! write(*,*) "coordenada de partida = ",ro/AU
            ! write(*,*) "indices de partida= ", i1, j1, k1
            ! write(*,*) "valor agregado = ", N_sum
            if (N_sum == 0) then
              write(*,*) ro/AU
              stop
            end if
            ! write(*,*) "ocoordenada de llegada= ", r/AU

            ! if (i1 >= 0 .and. i1 <= mapcells_x .and. j1 >= 0 .and. j1 <= mapcells_y) then
            ! write(*, *) "Posicion de partida del rayo(bloque)", i, j, k
            ! write(*, *) "Posicion de partida del rayo(global)", i1, j1, k1
            ...
            !       k2 = k1 + k_off
                  ! write(*,*) "N_sum ", N_sum
                  call projectCell(N_sum, i1, j1, k1)
            ! read(*,*)
            ! write(*, *) "Posicion de partida del rayo(global)", i1, j1, k1
